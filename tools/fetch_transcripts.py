...
def fetch_transcripts(downloader, entries, corpus_dir, delay=2.0, slug_filter=None):
    ...
    print(f"\nDone: {stats['ok']} fetched, {stats['skipped']} skipped, "
          f"{stats['error']} errors, {stats['total']} total")


def main():
    parser = argparse.ArgumentParser(
        description='Fetch EN+UK transcripts for talks in the corpus index'
    )
    parser.add_argument(
        '--index', default='glossary/corpus/index.yaml',
    ...
    fetch_transcripts(
        downloader, entries, corpus_dir,
        delay=args.delay, slug_filter=args.slug,
    )
