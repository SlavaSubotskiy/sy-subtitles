...
def format_stats(stats, label=""):
    ...
    lines.append(f"  CPS > target: {stats['cps_over_target']} ({stats['cps_over_target'] / stats['total_blocks'] * 100:.1f}%)")
    lines.append(f"  CPS > hard max: {stats['cps_over_hard']} ({stats['cps_over_hard'] / stats['total_blocks'] * 100:.1f}%)")
    ...
    lines.append(f"  Chars > max block: {stats['chars_over_max']} ({stats['chars_over_max'] / stats['total_blocks'] * 100:.1f}%)")
